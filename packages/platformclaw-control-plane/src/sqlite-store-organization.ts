...
function boundedReason(value: string, field: string): string {
...
export abstract class SqliteControlPlaneOrganizationStore extends SqliteControlPlaneOrganizationMemoryLifecycleStore {
  private organizationAuthorizationFacts(
    actorUserId: string,
    scopeId?: string,
    self = false,
  ): { source: "administrator" | "leadership" | "self" | "none"; scopeIds: string[] } {
  ...
  private insertOrganizationAudit(params: {
    actorUserId: string;
    action: string;
    targetType: string;
    targetId: string;
    ...
    scopeId?: string;
    self?: boolean;
    authorization?: OrganizationAuthorization["facts"];
    reason?: string;
  ...
  private runOrganizationMutation<T>(
    audit: {
      actorUserId: string;
      action: string;
      targetType: string;
  ...
  async archiveManagedScope(params: {
    ...
    return this.runOrganizationMutation(
      ...
      () => {
        ...
        const auditReason = boundedReason(params.reason, "reason");
