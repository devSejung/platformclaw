...
| board      | 별도 예약·상태 확인               | owner-aware MCP lease·control·evidence |
...
실제 engineer가 Web personal flow를 수행하고, Knox DM과 Group policy를 확인하며, 개인 개발 VM code change → build와 Board Farm MCP lease → deploy → boot → control → validate → Jira → Knox가 한 correlation ID로 연결돼야 한다. 최소 1개 failure/retry proof와 sanitized evidence가 있어야 business value를 actual로 평가할 수 있다.
