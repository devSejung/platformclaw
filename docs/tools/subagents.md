...
- **Run timeout:** pass `runTimeoutSeconds` to set a timeout for a specific native, ACP, or visible sub-agent run. When omitted, OpenClaw uses `agents.defaults.subagents.runTimeoutSeconds` if configured; otherwise it defaults to `1200` seconds (20 minutes). An explicit `0` disables the timeout for that run. ACP sessions retain a positive spawn timeout as the deadline for subsequent turns. This does not change `sessions_send` result-wait or ACP control-operation timeouts.
