...
export PLATFORMCLAW_SKILL_HUB_TOKEN_SECRET_FILE="$work_dir/skillhub-token"
export PLATFORMCLAW_SKILL_HUB_POSTGRES_PASSWORD_SECRET_FILE="$work_dir/skillhub-postgres-password"
export PLATFORMCLAW_SKILL_HUB_COOKIE_SECRET_FILE="$work_dir/skillhub-cookie-secret"
export PLATFORMCLAW_SKILL_HUB_BOOTSTRAP_PASSWORD_SECRET_FILE="$work_dir/skillhub-bootstrap-password"
...
touch "$PLATFORMCLAW_SKILL_HUB_TOKEN_SECRET_FILE"
openssl rand -hex 32 >"$PLATFORMCLAW_SKILL_HUB_POSTGRES_PASSWORD_SECRET_FILE"
openssl rand -hex 32 >"$PLATFORMCLAW_SKILL_HUB_COOKIE_SECRET_FILE"
openssl rand -base64 36 >"$PLATFORMCLAW_SKILL_HUB_BOOTSTRAP_PASSWORD_SECRET_FILE"
...
docker run --rm --network none --read-only --user 0:0 \
  ...
  --volume "$work_dir:/work" --entrypoint sh "$PLATFORMCLAW_IMAGE" -ceu \
...
docker run --rm --network "${project_name}_platformclaw-skillhub" --read-only \
  ...
  --volume "$PLATFORMCLAW_SKILL_HUB_BOOTSTRAP_PASSWORD_SECRET_FILE:/run/secrets/bootstrap-password:ro" \
  --volume "$PLATFORMCLAW_SKILL_HUB_TOKEN_SECRET_FILE:/run/secrets/control-token" \
