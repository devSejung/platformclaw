...
describe("PlatformClaw Docker runtime", () => {
  ...
  it("automatically embeds the canonical local pip config only in the sandbox image", () => {
    ...
    expect(build).toContain("PLATFORMCLAW_APT_SOURCES_SHA256=${aptSourcesSha");
    ...
    expect(sandboxDockerfile).toContain("/run/secrets/platformclaw_apt_sources | sha256sum -c -");
    expect(readRepoFile("Dockerfile.jammy")).toContain(
      "/run/secrets/platformclaw_apt_sources | sha256sum -c -",
    );
