...
PlatformClaw는 각 엔지니어에게 독립된 Assistant, Workspace, Execution Target과 정책 경계를 제공하고, 인증된 요청을 개인 또는 그룹 Agent로 연결한 뒤 빌드·Board Farm 검증·증거·Jira·Knox 결과까지 추적하는 멀티유저 AI 엔지니어링 플랫폼이다.
...
| ---------------------- | ---------------------------------------------------- | ---------------------------------------------------------- |
| Firmware/SoC engineer  | 채팅·IDE·VM·build server·Board Farm·Jira를 수동 전환 | 한 요청과 Run ID로 실행·검증·보고 연결                     |
...
3. build 성공과 hardware validation 사이의 lease, retry, evidence 책임이 분산된다.
...
- 검증 담당자로서 build 실패 시 보드가 점유되지 않고, lease timeout·validation 실패에도 evidence가 남기를 원한다.
...
Web login 또는 Knox DM이 directory identity를 개인 Agent의 `main` session에 연결한다. 사용자는 `platform_server` 또는 승인된 `assigned_vm` 중 준비된 대상을 선택한다. Target revision은 Run에 고정되며 자격 증명은 브라우저·transcript·Workspace에 기록되지 않는다.
...
| FR-09 | build 성공 전 Board Farm lease를 금지하고 lease lifecycle을 기록한다              | `MOCK_VERIFIED`                 | `packages/platformclaw-control-plane/src/board-farm/`, Mock evidence |
| FR-10 | 실제 Board Farm·Jira·Knox flow를 내부 adapter/Skill로 연결한다                    | `INTERNAL_INTEGRATION_REQUIRED` | `IR-001`~`IR-007`                                                    |
...
- Telegram/WhatsApp/Discord/Slack을 해커톤 핵심 surface로 홍보: `OUT_OF_SCOPE`
...
- 실제 VM code change와 build, Board Farm lease/deploy/boot/validate가 같은 Run으로 연결
