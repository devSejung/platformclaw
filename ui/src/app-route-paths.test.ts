...
describe("Dynamic route startup bridge", () => {
  ...
  it("redirects disabled dynamic and static history destinations to chat", async () => {
    let location: RouteLocation = { pathname: "/chat", search: "", hash: "" };
    let historyListener: ((next: RouteLocation) => void) | undefined;
    ...
    const router = createApplicationRouter(["chat"]);
    const chatRoute = router.getRoute("chat");
    if (!chatRoute) {
      throw new Error("Chat route missing");
    }
    const originalLoader = chatRoute.loader;
    const originalComponent = chatRoute.component;
    chatRoute.component = async () => ({ render: () => null });
    chatRoute.loader = vi.fn(async () => ({}));

    await startApplicationRouter(router, history, "", {
      basePath: "",
    } as unknown as ApplicationContext);
    location = { pathname: "/settings/agents/main/tools", search: "?probe=1", hash: "#tools" };
    historyListener?.(location);

    await vi.waitFor(() => expect(router.getState().location.pathname).toBe("/chat"));
    ...
    replace.mockClear();
    location = { pathname: "/sessions", search: "?probe=2", hash: "#sessions" };
    historyListener?.(location);
    expect(replace).toHaveBeenLastCalledWith({
      pathname: "/chat",
      search: "?probe=2",
    ...
    router.stop();
    chatRoute.loader = originalLoader;
    chatRoute.component = originalComponent;
  });
});
