...
export function renderSkillHubAdmin(props: {
  ...
  return html`<openclaw-modal-dialog
    ...
    @modal-cancel=${(event: Event) => (props.busy ? event.preventDefault() : props.onClose())}
  >
    <section class="skill-hub-dialog skill-hub-admin">
      ...
      ${!props.error && props.message
        ? html`<div class="callout success" role="status">${props.message}</div>`
        : nothing}
      <fieldset ?disabled=${props.busy} style="border:0;margin:0;padding:0;min-width:0">
        ${props.loading
                          ...
                          <div>
                            ...
                            ${binding.accessState === "restricted"
                              ? html`<span class="chip"
                                  >${t("skillHubPage.globalRestricted")}</span
                                >`
                              : nothing}
                            <small
