...
class SessionsPage extends OpenClawLightDomElement {
  ...
  private readonly subscriptions = new SubscriptionsController(this)
    ...
    .effect(
      ...
      (agentSelection) => {
        const enforcePersonalScope = () => {
          const context = this.context;
          const personalAgentId =
            context?.accessMode === "personal-agent"
              ? resolveSessionNavigationAgentId(context)
              : null;
          if (personalAgentId && agentSelection.state.scopeId !== personalAgentId) {
            agentSelection.setScope(personalAgentId);
          }
        };
        enforcePersonalScope();
        const stopEnforcing = agentSelection.subscribe(enforcePersonalScope);
        const stopObserving = this.observeAgentScope(agentSelection);
        return () => {
          stopEnforcing();
          stopObserving();
        };
  ...
  private sessionListOptions() {
    ...
    const context = this.context;
    const selectedScopeId = context?.agentSelection.state.scopeId ?? undefined;
    const scopeAgentId =
      context?.accessMode === "personal-agent"
        ? resolveSessionNavigationAgentId(context)
        : selectedScopeId;
    const queryAgentId =
      context?.accessMode === "personal-agent"
        ? scopeAgentId
        : deepLinkKey
          ? this.sessionAgentId(deepLinkKey)
          : scopeAgentId;
    return {
      activeMinutes:
      ...
      ...(queryAgentId ? { agentId: queryAgentId } : {}),
    };
  }
