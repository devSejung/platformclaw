...
function renderDiscover(props: PluginsViewProps) {
  ...
  const connectors =
    props.showMcp === false
      ? shelves.connectors.filter((connector) => connector.action.kind !== "mcp")
      : shelves.connectors;
  ...
  if (!featuredRows.length && !officialRows.length && !connectors.length) {
    return html`
      ${clawHub === nothing
        ? renderEmpty(
  ...
  return html`
    ...
    ${renderConnectorSection(connectors, props)} ${clawHub}
  `;
}
