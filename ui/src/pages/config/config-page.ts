...
export class ConfigPage extends OpenClawLightDomElement {
  ...
  private synchronizeRuntimeConfig(runtimeConfig: ApplicationContext["runtimeConfig"]) {
    ...
    // Personal-agent appearance is browser-local and must not probe operator config.
  ...
  private clearCustomTheme() {
    ...
    this.applySettings({
      ...this.settings,
      theme: this.settings.theme === "custom" ? UI_APPEARANCE_DEFAULTS.theme : this.settings.theme,
    ...
    const props: ConfigProps = {
      ...
      ...(this.usesBrowserOnlyPreferences()
        ? {
            ...
            localeOverride: isSupportedLocale(this.settings.locale)
