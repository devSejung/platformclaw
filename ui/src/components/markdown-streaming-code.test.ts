...
describe("streaming Markdown code fences", () => {
  ...
  it.each(["html", ""])("keeps open %j fences escaped and copyable", (language) => {
    const source = '<img src="x" onerror="alert(1)">';
    const fragment = htmlFragment(toStreamingMarkdownHtml(`\`\`\`${language}\n${source}`));
    ...
    expect(fragment.querySelector<HTMLElement>(".code-block-copy")?.dataset.code).toBe(source);
  });
