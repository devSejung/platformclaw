...
describe("normalizeInitialApplicationLocation", () => {
  ...
  it("ignores restricted pre-start navigation without replacing an enabled route", async () => {
    const previousSettings = loadSettings();
    const previousUrl = window.location.href;
    window.history.replaceState({}, "", "/new");
    const sessionPathBuilder = deferred<void>();
    const runtime = bootstrapApplication({
      enabledRouteIds: ["chat", "new-session"],
      sessionPathBuilderReady: sessionPathBuilder.promise,
    });

    try {
      const start = runtime.start();
      runtime.context.navigate("about");
      sessionPathBuilder.resolve();
      await start;
