...
describe("agent selection", () => {
  ...
  it("preserves an explicit scope when the same client's default agent arrives", () => {
    ...
    const client = { request() {} } as unknown as GatewayBrowserClient;
    gateway.publish({ client, assistantAgentId: "Main" });
    const selection = createAgentSelectionCapability(gateway.gateway, createRoster().roster);
    selection.setScope("Ops");

    gateway.publish({ client, assistantAgentId: "Person_One" });
  ...
  it("preserves a distinct explicit scope when the selected owner is removed", () => {
    const harness = createGateway("Main");
    const roster = createRoster();
    roster.publish({
      defaultId: "main",
    ...
    expect(selection.state).toEqual({ selectedId: "main", scopeId: "ops" });
  });

  it("repairs an explicit scope removed from the roster", () => {
    const harness = createGateway("Main");
    const roster = createRoster();
    roster.publish({
      defaultId: "main",
  ...
  it("repairs removed selection and scope during reconnect before hello", () => {
    const harness = createGateway("Main");
    const roster = createRoster();
    roster.publish({
      defaultId: "main",
