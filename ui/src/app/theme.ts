...
export type ThemeName = "platformclaw" | "claw" | "knot" | "dash" | "custom";
...
const VALID_THEME_NAMES = new Set<ThemeName>(["platformclaw", "claw", "knot", "dash", "custom"]);
...
export function parseThemeSelection(
  ...
  const normalizedTheme = VALID_THEME_NAMES.has(theme as ThemeName)
    ? (theme as ThemeName)
    : "platformclaw";
  // Keep the legacy parser fallback for persisted records that predate themeMode.
  // Fresh profiles get the downstream default from settings.ts instead.
...
export function resolveTheme(theme: ThemeName, mode: ThemeMode): ResolvedTheme {
  ...
  if (theme === "platformclaw") {
    return resolvedMode === "light" ? "platformclaw-light" : "platformclaw";
  }
  if (theme === "claw") {
