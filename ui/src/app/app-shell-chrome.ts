...
export class ShellChromeOwner {
  ...
  readonly handleNativeNavigate = (event: Event): void => {
    ...
    if (!routeId || !this.host.context || !this.host.enabledRouteIds().includes(routeId)) {
  ...
  readonly handleDocumentKeydown = (event: KeyboardEvent): void => {
    const host = this.host;
    if (!host.runtime?.shellSession && !host.commandPalette && isCommandPaletteShortcut(event)) {
      event.preventDefault();
      this.togglePalette();
      return;
    }
