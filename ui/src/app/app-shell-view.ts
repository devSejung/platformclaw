...
export function renderApplicationShell(host: ShellViewHost) {
  ...
  if (!settingsTakeover) {
    Object.assign(host.navigationSidebar, {
      ...
      onOpenApprovals: operatorChrome ? () => host.openApprovals() : undefined,
  ...
  return html`
    ${operatorChrome && isOptionalElementDefined(host.commandPaletteElement)
    ...
    >
      ...
      ${operatorChrome && isNativeWebChromeHost() && !onboarding
        ? html`
      ...
      <openclaw-app-topbar
        ...
        .onOpenPalette=${operatorChrome ? () => host.openPalette() : undefined}
        .onToggleDrawer=${(trigger: HTMLElement) => host.toggleNavigationSurface(trigger)}
      ></openclaw-app-topbar>
      ${operatorChrome && !onboarding && !settingsTakeover && !mobileNavLayout
        ? html`
      ...
      >
        ...
        ></openclaw-router-outlet>
        ${runtime.shellSession?.renderPageOverlay?.(activeRoute, host.routeState.location) ??
        nothing}
      </main>
