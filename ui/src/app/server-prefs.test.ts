...
describe("changedServerUiPrefs", () => {
  ...
  it("syncs an authored default-valued reset through the settings listener", async () => {
    ...
    const state = resolveServerUiPrefState(
      configWithPrefs({ theme: "platformclaw" }),
      "theme",
      scope,
    );
...
describe("pushServerUiPrefs", () => {
  ...
  it("keeps a synced default reset as a pending offline null intent", () => {
    ...
    expect(
      resolveServerUiPrefState(configWithPrefs({ theme: "platformclaw" }), "theme", scope),
    ).toEqual({
      overridden: false,
      provenance: "pending",
      resetValue: "platformclaw",
