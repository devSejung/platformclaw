...
class OpenClawApp extends OpenClawLightDomElement {
  ...
  override render() {
    ...
    const showLoginGate =
      ...
      !gatewayConnected &&
      (this.loginGatePinned || gatewaySnapshot.phase !== "reconnecting");
    if (showLoginGate) {
      return html`
        <openclaw-tooltip-provider>
          <openclaw-login-gate
...
class OpenClawShell extends OpenClawLightDomElement {
  ...
  override connectedCallback() {
    ...
    setSettingsChangeListener((previous, next) => {
      if (isApplyingServerUiPrefs() || this.context?.accessMode !== "operator") {
        return;
      }
  ...
  private readonly handleGatewayEvent = (event: GatewayEventFrame) => {
    ...
    if (event.event === "config.changed") {
      ...
      const runtimeConfig =
        this.context?.accessMode === "operator" ? this.context.runtimeConfig : undefined;
  ...
  private readonly handleNativeNavigate = (event: Event) => {
    ...
    if (!routeId || !this.context || !this.enabledRouteIds().includes(routeId)) {
  ...
  private readonly handleDocumentKeydown = (event: KeyboardEvent) => {
    if (!this.runtime?.shellSession && !this.commandPalette && isCommandPaletteShortcut(event)) {
      event.preventDefault();
      this.togglePalette();
      return;
    }
  ...
  override render() {
    ...
    if (!settingsTakeover) {
      Object.assign(this.navigationSidebar, {
        ...
        onOpenApprovals: operatorChrome ? this.openApprovals : undefined,
    ...
    return html`
      ...
      >
        ...
        ${isNativeWebChromeHost() && !onboarding && operatorChrome
          ? html`
