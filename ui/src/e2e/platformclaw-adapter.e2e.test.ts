...
describeControlUiE2e("PlatformClaw Control UI adapter mocked Gateway E2E", () => {
  ...
  it("keeps personal exec credentials separate from MCP and hides unavailable settings", async () => {
    const { page } = await newPage();
    await installPlatformClawDocument(page);
    await page.route("**/platformclaw/api/auth/session", (route) =>
      route.fulfill({ json: activeSession(), status: 200 }),
    );
    await page.route("**/platformclaw/api/exec-credentials", (route) =>
      route.fulfill({
        json: { definitions: [{ envName: "API_TOKEN", configured: false }] },
        status: 200,
      }),
    );
    await page.route("**/platformclaw/api/mcp", (route) =>
    ...
    const settingsSidebar = page.locator(".settings-sidebar");
    await expect.poll(() => settingsSidebar.isVisible()).toBe(true);
    expect(await settingsSidebar.getByRole("link", { name: "Ask OpenClaw" }).count()).toBe(0);
    expect(await settingsSidebar.getByRole("link", { name: "Privacy & Security" }).count()).toBe(0);
    expect(await settingsSidebar.getByRole("link", { name: "Advanced" }).count()).toBe(0);

    await settingsSidebar.getByRole("link", { name: "Credentials", exact: true }).click();
    await expect
      .poll(() => new URL(page.url()).pathname)
      .toBe("/platformclaw/app/settings/credentials");
    const credentials = page.locator("platformclaw-exec-credentials");
    await expect
      .poll(() =>
        credentials.getByRole("heading", { name: "Personal exec credentials" }).isVisible(),
      )
      .toBe(true);
    await expect
      .poll(() => credentials.getByText("API_TOKEN", { exact: true }).isVisible())
      .toBe(true);

    if (captureUiProofEnabled) {
      await page.screenshot({
        fullPage: true,
        path: path.join(proofDir, "04-personal-exec-credentials.png"),
      });
    }
    ...
    await expect
      .poll(() =>
        page
          ...
          .getByRole("heading", { name: "Your MCP credentials" })
          .isVisible(),
      )
      .toBe(true);
    await expect.poll(() => page.locator("platformclaw-exec-credentials").isVisible()).toBe(false);
  });
