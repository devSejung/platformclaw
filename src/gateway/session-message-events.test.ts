...
describe("session.message websocket events", () => {
  ...
  test("publishes a current cron completion live and restores it from WebChat history", async () => {
    const storePath = await createSessionStoreFile();
    ...
    try {
      ...
      const dispatched = await dispatchCronDelivery({
        ...
        job: {
          ...
          sessionKey,
          owner: { agentId: "main", sessionKey, accountId: "owner@example.test" },
          wakeMode: "now",
          enabled: true,
          state: {},
          createdAtMs: 1,
        ...
        deliveryPayloads: [{ text: "The detached cron finished automatically." }],
        synthesizedText: "The detached cron finished automatically.",
        summary: "The detached cron finished automatically.",
        outputText: "The detached cron finished automatically.",
        isAborted: () => false,
        abortReason: () => "aborted",
      ...
      expect(livePayload.message).toMatchObject({
        __openclaw: { idempotencyKey: "cron-current-completion:cron:job-webchat:3000" },
        content: [{ type: "text", text: "The detached cron finished automatically." }],
        openclawAutomation: {
          kind: "cron",
          jobId: "job-webchat",
