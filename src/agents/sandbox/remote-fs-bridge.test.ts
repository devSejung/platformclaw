...
describe("remote sandbox fs bridge", () => {
  it("keeps skill paths read-only under a broader writable remote root", async () => {
    await withTempDir("openclaw-remote-protected-skill-", async (workspaceDir) => {
      await fs.mkdir(path.join(workspaceDir, "skills"), { recursive: true });
      const bridge = createRemoteShellSandboxFsBridge({
        sandbox: createSandbox({ workspaceDir, agentWorkspaceDir: workspaceDir }),
        runtime: {
          remoteWorkspaceDir: "/users/worker/.platformclaw/workspace",
          remoteAgentWorkspaceDir: "/users/worker/.platformclaw/workspace",
      ...
      await expect(
        bridge.writeFile({
          filePath: "/users/worker/.platformclaw/workspace/skills/policy/SKILL.md",
          data: "tampered",
        }),
