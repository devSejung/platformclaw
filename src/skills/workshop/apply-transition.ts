...
import {
  applyExternalSkillMutationAndCommit,
  createSkillProposalRollback,
  reconcileExternalSkillMutation,
} from "./apply-transition-external.js";
...
import { assertSkillWorkshopTargetAccess } from "./target-access.js";
    ...
    async () => {
      ...
      if (record.target.binding) {
        return await applyExternalSkillMutationAndCommit({
          ...
          operations: {
            markStale: markSkillProposalStale,
            assertSupportTargetUnchanged: assertSkillProposalSupportTargetUnchanged,
          },
        });
      }
